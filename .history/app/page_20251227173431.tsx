...
<div className="max-w-6xl mx-auto px-4 mb-10 flex flex-col md:flex-row items-end gap-8">
  ...
  <div className="flex-1 w-full bg-white p-5 rounded-2xl shadow-sm border border-gray-100">
    ...
    <Range
      ...
      min={MIN}
      max={MAX}
      onChange={(newValues) => setValues(newValues)} // Cập nhật giá trị khi kéo
        ...
        <div
          ...
          style={{
            ...
            background: getTrackBackground({
              values,
              colors: ["#fef9c3", "#facc15", "#fef9c3"], // Màu xám nhạt - vàng - xám nhạt
              min: MIN,
              max: MAX,
            }),
          }}
      ...
      renderThumb={({ props, isDragged }) => (
        <div
          {...props}
          style={{ ...props.style }}
          className={`h-6 w-6 rounded-full bg-white border-4 border-yellow-400 shadow-md outline-none transition-transform ${
            isDragged ? "scale-125" : "scale-100"
          }`}
        />
      )}
    />
