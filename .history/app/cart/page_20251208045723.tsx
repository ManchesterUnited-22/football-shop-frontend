...
// Chúng ta lấy cartItems, updateQuantity và removeFromCart từ useCart
import { useCart } from '../context/CartContext'; 
import Image from 'next/image'; // Nên dùng Image của Next.js nếu có thể

export default function CartPage() {
    // ⭐️ Lấy cartItems (đã đổi tên trong useCart) và các hàm cần thiết ⭐️
    const { cartItems, removeFromCart, updateQuantity } = useCart();

    // ⭐️ FIX LỖI TÍNH TỔNG TIỀN: Phải nhân với item.quantity ⭐️
...
                                    {item.imageUrl ? (
                                    // Thay thế <img> bằng <Image> (Next.js best practice)
                                        <Image 
                                            src={item.imageUrl} 
                                            alt={item.name} 
                                            layout="fill" 
...
                                <button 
                                  onClick={() => removeFromCart(item.id)} // Sử dụng item.id để xóa
...
                    {/* ⭐️ FIX LỖI CHUYỂN HƯỚNG: Sử dụng Link tới /checkout ⭐️ */}
