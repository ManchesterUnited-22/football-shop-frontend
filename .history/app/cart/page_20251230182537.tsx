  ...
  if (cartItems.length === 0) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-br from-green-800 via-black to-green-900 px-6">
        ...
        <h1 className="text-4xl font-extrabold text-white mb-4 text-center">Giỏ hàng trống</h1>
        <p className="text-gray-300 text-lg mb-10 text-center max-w-md">Hãy chọn ngay những món đồ bóng đá yêu thích để cháy hết mình trên sân cỏ!</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2 space-y-8">
            {cartItems.map((item) => (
              <div 
                key={item.id} 
                className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-3xl shadow-2xl overflow-hidden flex flex-col sm:flex-row gap-6 p-6 hover:scale-[1.02] hover:shadow-red-500/10 transition-all duration-300"
              >
                ...
                <div className="flex-grow flex flex-col justify-between">
                  ...
                  <div className="flex items-center justify-between mt-6">
                    {/* Quantity selector hiện đại */}
                    <div className="flex items-center bg-white/10 rounded-full px-4 py-2 shadow-inner">
                      <button
                        onClick={() => updateQuantity(item.id, item.quantity - 1)}
                        disabled={item.quantity <= 1}
                        className="p-2 text-gray-300 hover:text-white hover:bg-white/20 rounded-full transition-all disabled:opacity-50"
                      >
                        <Minus size={18} />
                      </button>
                      ...
                      <button
                        onClick={() => updateQuantity(item.id, item.quantity + 1)}
                        className="p-2 text-gray-300 hover:text-white hover:bg-white/20 rounded-full transition-all"
                      >
                        <Plus size={18} />
                      </button>
                    </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-gradient-to-b from-white/10 to-white/5 backdrop-blur-xl p-8 rounded-3xl shadow-2xl border border-white/20 sticky top-28">
              ...
              <Link href="/checkout">
                <button className="w-full bg-gradient-to-r from-red-600 to-red-800 text-white py-5 rounded-2xl font-extrabold text-xl hover:scale-105 hover:shadow-2xl transition-all duration-300 uppercase tracking-wider shadow-xl">
                  Thanh Toán Ngay
                </button>
              </Link>
