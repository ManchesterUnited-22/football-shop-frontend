...
interface SalesReportQuery {
  type: 'best' | 'worst';
  limit: number;
  period: '7days' | '30days' | '6months' | 'year';
}
interface Category {
    id: number;
    name: string;
...
    const [inputLimit, setInputLimit] = useState<string>('20');
    ...
    // Sử dụng loading chung để đơn giản
    setLoading(true); 
    setError(null);

    // Xây dựng URL với các tham số từ state
...
                <div className="flex flex-col space-y-3">
  <label
    htmlFor="limit"
    className="font-semibold text-gray-800 text-sm tracking-wide"
  >
    Giới hạn kết quả
  </label>
