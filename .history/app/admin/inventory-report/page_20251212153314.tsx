...
        }
        ...
        // 1. Xây dựng URL với tất cả 3 tham số (threshold, limit, categoryId)
...
        try {
            // 2. GỌI API VỚI URL ĐÃ XÂY DỰNG ĐẦY ĐỦ
...
    // useEffect để tải dữ liệu báo cáo (chạy lại khi threshold, category, hoặc limit thay đổi)
    useEffect(() => {
        if (!isLoading && isAuthenticated) {
            setLoading(true); 
            fetchLowStockData();
        }
    }, [isLoading, isAuthenticated, threshold, selectedCategory, limit]); // ✅ CÓ limit
...
            
            {/* KHUNG LỌC (Ngưỡng + Danh mục + Limit) */}
            <div className="flex flex-wrap items-end gap-6 mb-6 p-4 bg-gray-50 border rounded-lg">
                
                {/* LỌC THEO NGƯỠNG */}
                <div className="flex flex-col space-y-2">
                    <label htmlFor="threshold" className="font-medium text-gray-700 whitespace-nowrap text-sm">Ngưỡng Tồn Kho (&lt;)</label>
...
                            className="px-3 py-2 bg-red-600 text-white font-semibold rounded-md hover:bg-red-700 transition duration-150"
                            disabled={loading}
                        >
                            Áp dụng
                        </button>
...
                {/* LỌC THEO DANH MỤC */}
                <div className="flex flex-col space-y-2">
                    <label htmlFor="category" className="font-medium text-gray-700 whitespace-nowrap text-sm">Lọc theo Danh mục</label>
                    <select
                        id="category"
...
                    <p className="mb-4 text-gray-700">
                        Danh sách {reportData.length} mặt hàng có tồn kho thấp nhất (dưới {threshold + 1})
...
                        <h3 className="text-xl font-semibold mb-4">Biểu đồ Tồn Kho Thấp nhất (Ngưỡng: {threshold} / Giới hạn: {limit})</h3>
