...
interface Product {
  ...
  salePrice: number | null; // Giá sau khi giảm (nếu có)
}
...
export default function AdminPage() {
  ...
  const handleSaveGlobalPromo = async () => {
    ...
    try {
      ...
      fetchProducts(); // Tải lại để cập nhật giá hiển thị
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-100 p-4 sm:p-10">
      <div className="max-w-7xl mx-auto space-y-10">
        ...
        <div className="flex flex-col md:flex-row gap-10">
            ...
            <div className="space-y-4 max-h-[700px] overflow-y-auto pr-2">
              {products.map((p) => {
                ...
                return (
                  <div
                    key={p.id}
                    className="flex items-center justify-between p-4 border rounded-xl hover:bg-gray-50 transition-all relative overflow-hidden group"
                  >
                    <div className="flex items-center gap-4 flex-grow min-w-0">
                      {/* ẢNH SẢN PHẨM + BADGE SALE */}
                      <div className="relative w-20 h-20 flex-shrink-0">
                        ...
                        {/* HIỂN THỊ BADGE SALE TRÊN ẢNH */}
                        {isSale && (
                          <span className="absolute top-0 left-0 bg-red-600 text-white text-[10px] font-black px-2 py-0.5 rounded-br-lg shadow-md z-10 animate-bounce-subtle">
                            SALE
                          </span>
                        )}
                      </div>
                      ...
                      <div className="min-w-0 flex-grow">
                        <p className="font-bold text-gray-800 text-base truncate group-hover:text-blue-600 transition-colors">
                            {p.name}
                        </p>
                        ...
                        <div className="mt-1">
                          {isSale ? (
                            <div className="flex flex-col">
                              ...
                              {p.promoName && (
                                <p className="text-[10px] text-orange-500 font-bold italic uppercase">
                                  ⚡ {p.promoName}
                                </p>
                              )}
                            </div>
          ...
          {/* FORM CHỈNH SỬA NHANH + TÁC VỤ (GIỮ NGUYÊN) */}
            ...
            {editingProduct ? (
              <div className="space-y-5">
                ...
                <div className="text-center text-gray-500 py-10">
                  <p>Hệ thống đang mở chế độ chỉnh sửa...</p>
                  <button
                    onClick={cancelEdit}
              ...
              <div className="space-y-5">
                ...
                <Link
                  ...
                  className="block text-center py-4 bg-yellow-500 hover:bg-yellow-600 text-white font-bold rounded-xl shadow transition-all flex items-center justify-center gap-3 text-lg"
                >
                  <ClipboardCheck size={24}/> Xem báo cáo kho
                </Link>
              </div>
            )}
          </div>
