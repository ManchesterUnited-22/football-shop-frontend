...
export default function AddToCartBtn({
  ...
  return (
    <section className="min-h-screen w-full bg-neutral-950 text-white">
      <div className="max-w-[1200px] mx-auto p-10 space-y-12">
        
        {/* Ảnh sản phẩm */}
        <div className="w-full flex justify-center items-center bg-neutral-900 p-6 rounded-xl">
          <img
            src={product.images?.[0] || '/placeholder.jpg'}
            alt={product.name}
            className="w-full max-w-[1000px] h-auto object-contain brightness-95 contrast-105 rounded-xl"
          />
        </div>

        {/* Tên sản phẩm */}
        <h1 className="text-3xl lg:text-4xl font-extrabold uppercase tracking-wide">
          {product.name}
        </h1>

        {/* Giá sản phẩm */}
        <div className="flex items-end gap-6">
          <span className={`text-5xl font-extrabold ${isOnSale ? 'text-red-500' : 'text-green-400'}`}>
              ...
              {uniqueSizes.map(sizeValue => {
                ...
                return (
                  <button
                    ...
                    disabled={disabled}
                    className={`w-20 h-20 rounded-xl font-bold text-xl transition-all duration-300 shadow-lg border
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8">
          <div className="flex items-center gap-8">
            <span className="text-xl font-bold uppercase tracking-wider">Số lượng</span>
            <div className="flex items-center bg-neutral-800 rounded-xl p-3 shadow-lg border border-neutral-700">
              <button
                onClick={() => handleQuantityChange('decrement')}
                disabled={quantity <= 1}
                className="p-4 rounded-lg hover:bg-neutral-700 transition disabled:opacity-50"
              >
                <Minus size={28} className="text-gray-300" />
              </button>
              <span className="w-24 text-center text-4xl font-extrabold">
                {quantity}
              </span>
              <button
        ...
        {/* Giá tạm tính */}
        <div className="bg-neutral-900 p-8 rounded-xl border border-neutral-700 shadow-lg">
          <p className="text-gray-400 text-lg uppercase font-bold tracking-wider mb-4">
            Giá tạm tính ({quantity} sản phẩm)
          </p>
          <div className="flex items-end gap-6">
            <span className={`text-4xl lg:text-5xl font-extrabold ${isOnSale ? 'text-red-500' : 'text-green-400'}`}>
        ...
        <button
          ...
          className={`w-full py-6 rounded-xl font-extrabold text-2xl uppercase tracking-wide transition-all duration-500 flex items-center justify-center gap-4 shadow-lg
            ${isBuyDisabled
