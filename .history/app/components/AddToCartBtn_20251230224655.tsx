...
export default function AddToCartBtn({
  ...
  const handleAddToCart = () => {
    ...
    // Optional: Có thể thêm toast success ở đây sau
  };

  const isBuyDisabled = !currentVariant || currentVariant.stock <= 0;
    ...
    <div className="mt-8 space-y-8">
            ...
            {uniqueSizes.map(sizeValue => {
              ...
              return (
                <button
                  ...
                  disabled={disabled}
                  className={`w-16 h-16 rounded-2xl font-black text-lg transition-all duration-300 shadow-lg border-2
                    ${selectedSize === sizeValue 
                      ? 'bg-emerald-600 text-white border-emerald-500 shadow-emerald-600/50 scale-110' 
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <h4 className="text-lg font-bold text-gray-300 uppercase tracking-wider">Số lượng</h4>
        ...
        {currentVariant && (
          <span className={`text-lg font-bold ${stock > 5 ? 'text-emerald-400' : stock > 0 ? 'text-yellow-400' : 'text-red-400'}`}>
            Kho: {stock} sản phẩm
          </span>
        )}
      </div>
      ...
      <div className="p-6 bg-gray-800/50 backdrop-blur-sm rounded-3xl border border-gray-700">
        <p className="text-gray-400 text-sm uppercase font-bold tracking-wider mb-2">Giá tạm tính ({quantity} sản phẩm)</p>
        <div className="flex items-end gap-4">
          <span className={`text-4xl lg:text-5xl font-black ${isOnSale ? 'text-red-500' : 'text-emerald-400'}`}>
