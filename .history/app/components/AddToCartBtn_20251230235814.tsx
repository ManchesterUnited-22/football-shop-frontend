    ...
    <div className="space-y-8 lg:space-y-10 animate-fadeIn">
            ...
            {uniqueSizes.map(size => {
              ...
              return (
                <button
                  ...
                  className={`w-16 h-16 rounded-xl font-bold text-lg transition-all duration-300 border-2 shadow-md transform
                    ${selectedSize === size
                      ? 'bg-gradient-to-br from-emerald-500 to-green-600 text-white border-emerald-400 shadow-emerald-500/40 scale-105 animate-pulse'
                      : 'bg-slate-800/70 text-gray-300 border-slate-700 hover:border-emerald-500 hover:bg-slate-700/80 hover:scale-105'
                    }
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center bg-slate-800/70 rounded-xl p-2 border border-slate-700 shadow-inner">
            <button
              ...
              disabled={quantity <= 1}
              className="p-3 rounded-lg hover:bg-slate-700 transition disabled:opacity-50 active:scale-90"
            >
              <Minus size={24} className="text-gray-300" />
            </button>
            <span className="w-20 text-center text-3xl font-black text-white animate-fadeIn">
              {quantity}
            </span>
            <button
        ...
        {stock > 0 && stock < Infinity && (
          <span className={`text-lg font-bold animate-fadeIn ${stock > 10 ? 'text-emerald-400' : stock > 3 ? 'text-amber-400' : 'text-red-400'}`}>
            Còn {stock} sản phẩm
          </span>
        )}
      </div>
      ...
      {/* === GIÁ TẠM TÍNH === */}
      <div className="p-6 bg-slate-800/50 backdrop-blur-md rounded-2xl border border-slate-700 animate-slideUp">
        <p className="text-gray-400 text-sm uppercase font-bold tracking-wider mb-3">
          Giá tạm tính ({quantity} sản phẩm)
        </p>
        <div className="flex items-end gap-5">
          <span className={`text-4xl font-black ${isOnSale ? 'text-red-400 animate-pulse' : 'text-emerald-400'}`}>
      ...
      <button
        ...
        className={`w-full py-5 rounded-xl font-black text-xl uppercase tracking-wider flex items-center justify-center gap-4 transition-all duration-300 shadow-xl transform
          ${isBuyDisabled
            ? 'bg-slate-800 text-gray-500 cursor-not-allowed'
            : 'bg-gradient-to-r from-emerald-600 to-green-700 hover:from-emerald-700 hover:to-green-800 text-white shadow-emerald-600/50 hover:shadow-emerald-600/80 hover:-translate-y-1 active:scale-95 animate-bounceSlow'
          }
        `}
      >
