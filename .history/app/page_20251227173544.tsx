...
const CATEGORIES = [
  ...
  { id: 4, title: "Cầu lông & Pickleball", sub: "Cool", img: "/categories/pickleball.jpg" },
];
            ...
            <div className="px-2 pt-4">
              <Range
                ...
                renderTrack={({ props, children }) => (
                  <div {...props} className="h-1.5 w-full rounded-full" 
                    style={{ ...props.style, background: getTrackBackground({ values, colors: ["#f1f5f9", "#facc15", "#f1f5f9"], min: MIN, max: MAX }) }}>
                    {children}
                  </div>
                )}
                renderThumb={({ props, isDragged }) => (
                  <div {...props} style={{ ...props.style }} 
                    className={`h-6 w-6 rounded-full bg-white border-4 border-yellow-400 shadow-md outline-none transition-transform ${isDragged ? 'scale-125' : 'scale-100'}`} />
                )}
              />
