...
// ⚠️ CHÚ Ý: useAuth (Lấy token và trạng thái)
// Trong môi trường thực tế, hook này sẽ lấy dữ liệu từ session/token
// Tôi sẽ sửa đổi hook này để lấy token từ localStorage
const useAuth = () => {
    // Lấy trạng thái đăng nhập dựa trên sự tồn tại của Access Token
    if (typeof window !== 'undefined') {
        const token = localStorage.getItem('access_token');
        // Giả định bạn có cách lấy userId và role từ state/context, 
        // nhưng ở đây ta chỉ cần kiểm tra sự tồn tại của token.
        return { 
            isAuthenticated: !!token, 
            token: token // Trả về token để sử dụng khi gọi API
        };
    }
...
    
    // Lấy trạng thái User và TOKEN từ hook
...
            alert("Vui lòng đăng nhập để hoàn tất đơn hàng. Không tìm thấy token.");
            router.push('/login'); 
            return;
        }
...
        // 3. CHUẨN BỊ PAYLOAD (userId không cần gửi vì Backend lấy từ token)
...
        try {
            // 4. GỌI API BACKEND NESTJS VÀ THÊM HEADER AUTHORIZATION
            const response = await fetch('http://localhost:3001/orders', {
                method: 'POST',
                headers: { 
                    'Content-Type': 'application/json',
                    // ⭐️ FIX LỖI 401: THÊM TOKEN VÀO HEADER BEARER ⭐️
...
            router.push(`/user/order-confirmation/${result.id}`); 
