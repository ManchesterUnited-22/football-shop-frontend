  ...
  const addToCart = (itemDetails: CartItemDetails) => { 
   ...
   if (existingItemIndex > -1) {
        ...
        newItems[existingItemIndex] = {
            ...existingItem,
            quantity: existingItem.quantity + itemDetails.quantity, // Tăng lên 1 (hoặc số lượng được truyền vào)
        };
    } else {
        // 3. NẾU KHÔNG TÌM THẤY: Thêm item mới hoàn toàn
        const newItem: CartItem = {
            // Tốt nhất nên dùng một unique ID (ví dụ: Date.now() hoặc uuid) cho ID item trong giỏ, 
            // nhưng tạm thời dùng logic của bạn
            id: Date.now(), 
...
    
    alert(`✅ Đã thêm thành công: ${newItems.name} (Size ${newItems.sizeValue})`);
  };
