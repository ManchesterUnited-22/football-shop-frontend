...
export function useCart() {
    ...
    // Trả về đối tượng mở rộng (chứa cartCount và đổi tên items)
    return { 
        ...context, 
        cartCount, 
...
export function useCart() {
    ...
    // Tính tổng số lượng (ĐÃ CÓ)
    const cartCount = context.items.reduce((total, item) => total + item.quantity, 0);

    // ⭐️ HÀM MỚI: TÍNH TỔNG TIỀN ⭐️
    const cartTotalAmount = context.items.reduce((total, item) => {
        return total + (item.price * item.quantity); // Nhân giá * số lượng
    }, 0); 

    // Trả về đối tượng mở rộng
    return { 
        ...context, 
        cartCount, 
        cartTotalAmount, // ⭐️ Thêm Tổng tiền vào kết quả trả về ⭐️
        cartItems: context.items 
    };
}