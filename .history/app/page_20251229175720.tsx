...
// 2. Data Cố định
const CATEGORIES = [
  { id: 1, title: "Áo Câu Lạc Bộ", sub: "Club Kits", img: "/categories/ao-quan-bong-da.jpg" },
  { id: 2, title: "Áo Đội Tuyển", sub: "National Kits", img: "/categories/ao-quan-bong-da.jpg" }, // Bạn có thể đổi ảnh sau
  { id: 3, title: "Áo Training", sub: "Training Wear", img: "/categories/ao-quan-bong-da.jpg" },
  { id: 4, title: "Giày Đá Banh", sub: "Football Boots", img: "/categories/giay-the-thao.jpg" },
  { id: 5, title: "Giày Đá Banh", sub: "Football Boots", img: "/categories/giay-the-thao.jpg" },
];
...
export default function Home() {
  ...
  min={MIN}
  max={MAX}
  onChange={(newVals: number[]) => setValues(newVals)}
  renderTrack={({ props, children }) => {
    // 1. Tách key ra để không bị lỗi "Spread key"
    const { key, ...restProps } = props as any;
    return (
      <div
        key={key} // Gán key trực tiếp ở đây
        {...restProps}
        className="h-1.5 w-full rounded-full" // Đảm bảo có chiều cao h-1.5
        style={{
          ...restProps.style,
          background: getTrackBackground({
            values,
            colors: ["#f1f5f9", "#facc15", "#f1f5f9"],
            min: MIN,
            max: MAX,
          }),
        }}
  ...
  renderThumb={({ props, isDragged }) => {
    // 2. Tương tự, tách key cho nút kéo
    const { key, ...restProps } = props as any;
    return (
      <div
        key={key} // Gán key trực tiếp ở đây
        ...
        className={`h-6 w-6 rounded-full bg-white border-4 border-yellow-400 shadow-md outline-none transition-all ${
          isDragged ? "scale-125 shadow-xl" : "scale-100"
        }`}
      />
    );
  }}
/>
      ...
      {/* --- SECTION HÀNG MỚI VỀ (CHỈ HIỆN 4 CÁI MỚI NHẤT) --- */}
{products.some(p => p.isNew) && (
  <section className="max-w-6xl mx-auto px-4 py-8">
    <div className="flex items-center gap-4 mb-8">
      <h2 className="text-2xl font-black italic uppercase tracking-tighter">Hàng mới về</h2>
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
       {products.filter(p => p.isNew).slice(0, 4).map(p => (
         <div key={p.id} className="bg-white p-3 rounded-2xl shadow-sm border border-emerald-100 flex items-center gap-3">
            <img src={p.images[0]} className="w-16 h-16 object-cover rounded-lg bg-gray-50" />
            <div>
              <h4 className="text-xs font-bold truncate w-24">{p.name}</h4>
              <p className="text-emerald-600 font-black text-sm">{Number(p.currentPrice).toLocaleString()}đ</p>
            </div>
         </div>
       ))}
    </div>
        ...
        {products.map((product) => (
          <Link key={product.id} href={`/products/${product.slug}`} className="group">
            <div className="bg-white rounded-3xl p-5 shadow-sm hover:shadow-2xl transition-all duration-500 relative border border-gray-50 flex flex-col h-full">
              ...
              {product.isNew && (
              <div className="absolute top-4 right-4 z-30 bg-emerald-500 text-white px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest shadow-lg border border-white/20 animate-bounce">
              Hàng mới về
              </div>
              )}
