  ...
  return (
    <main className="min-h-screen bg-gray-100 relative overflow-hidden">
      ...
      <div className="relative flex items-center">
          <div className={`flex items-center bg-white border border-gray-300 rounded-lg shadow transition-all duration-500 overflow-hidden ${isSearchOpen ? 'w-full md:w-80 px-3' : 'w-12 h-12 justify-center cursor-pointer hover:bg-gray-50'}`}>
            
            {/* Icon Kính lúp (Nút bấm) */}
            <button 
            ...
            {/* Input nhập liệu (Chỉ hiện khi isSearchOpen = true) */}
            <input
              type="text"
              placeholder="Tìm giày, áo, phụ kiện..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className={`bg-transparent border-none focus:ring-0 text-gray-700 w-full ml-2 transition-opacity duration-300 ${isSearchOpen ? 'opacity-100' : 'opacity-0 w-0'}`}
            />
