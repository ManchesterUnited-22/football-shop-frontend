...
const CountdownTimer = ({ targetDate }: { targetDate: string }) => {
  ...
  return (
    <div className="flex items-center justify-center gap-1.5 text-[10px] md:text-[11px] font-black bg-black/70 text-white py-1.5 px-3 rounded-full backdrop-blur-md border border-white/20 shadow-lg">
      <span className="w-2 h-2 bg-red-500 rounded-full animate-ping"></span>
      <span className="tracking-tighter">
        KẾT THÚC SAU: {timeLeft.d > 0 ? `${timeLeft.d}N ` : ""}
          ...
          <Link key={product.id} href={`/products/${product.slug}`} className="group">
            <div className="bg-white rounded-2xl p-4 shadow-sm hover:shadow-xl transition-all duration-300 relative border border-gray-100 flex flex-col h-full">
              ...
              <div className="h-64 rounded-xl mb-4 overflow-hidden bg-[#f3f4f6] relative">
                {/* HIỂN THỊ COUNTDOWN NẾU ĐANG SALE */}
                  {product.isSale && product.promoEnd && (
                    <div className="absolute bottom-3 left-1/2 -translate-x-1/2 z-20 w-[90%] pointer-events-none">
                      <CountdownTimer targetDate={product.promoEnd} />
                    </div>
                  )}
                <img
